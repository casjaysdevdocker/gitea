...
if [ -n "$SERVER1_APPNAME_ENABLED" ]; then
  if [ "$SERVER1_APPNAME_ENABLED" != "yes" ]; then
...
#SERVICE_USER="server1"
# Set the service group
#SERVICE_GROUP="server1"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Set password length
RANDOM_PASS_USER=""
RANDOM_PASS_ROOT=""
...
# root user name
root_user_name="${SERVER1_ROOT_USER_NAME:-}"
# root user password
root_user_pass="${SERVER1_ROOT_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Normal user info [password/random]
# normal user name
user_name="${SERVER1_USER_NAME:-}"
# normal user password
user_pass="${SERVER1_USER_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Load variables from config
# Generated by my dockermgr script
if [ -f "/config/env/server1.script.sh" ]; then
  . "/config/env/server1.script.sh"
fi
# Overwrite the variables
if [ -f "/config/env/server1.sh" ]; then
  . "/config/env/server1.sh"
fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # replace variables
  # __replace "" "" "$CONF_DIR/server1.conf"
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$SERVER1_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$SERVER1_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$SERVER1_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$SERVER1_USER_PASS_WORD}"             #
